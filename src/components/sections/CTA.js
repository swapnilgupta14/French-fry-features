...
const ContactUsSection = () => {
  ...
  return (
    <div
      ref={sectionRef}
      className="relative h-[80vh] bg-gray-300 cursor-pointer overflow-hidden flex items-center justify-center"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      <img
        className={`absolute inset-0 w-full h-full object-cover filter  transition-transform duration-700 ease-in-out ${
      ...
      <div className="relative z-10 text-center text-accent p-8 bg-white bg-opacity-80 rounded-lg">
        <h2 className="text-3xl lg:text-4xl xl:text-5xl font-bold mb-6 leading-tight custom-font">
          Let's Collaborate
        </h2>
        <p className="text-lg lg:text-lg xl:text-xl mb-8 max-w-3xl mx-auto leading-relaxed">
          Transform your digital presence with our expert team. We turn
          innovative ideas into stunning realities. Our comprehensive approach
          combines strategic thinking and creative design to deliver exceptional
          digital solutions.
        </p>

        <div className="w-full flex justify-center">
          <Link href="/connect">
            <button className="bg-accent text-white py-3 px-6 rounded-full font-semibold hover:bg-gray-100 hover:text-black transition duration-300 flex items-center justify-center">
              <Send className="mr-3" /> Contact Us
            </button>
          </Link>
        </div>
